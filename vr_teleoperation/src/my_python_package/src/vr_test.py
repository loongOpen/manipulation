...
from mocap2robot_src.HelpFuction import matrix3d_to_euler_angles_zyx, xyz_quaternion_to_homogeneous, rpy2rotation_matrix, rotation_matrix_to_rpy, \
    find_axis_angle, calc_dist,fast_mat_inv
...
def calibrate():
    ...
    if vr_device_type=='vision_pro':
        ...
        quest3_hand.calibrate_shoulder()
    else:
...
def get_hand_tf(group_to_head,group_to_left_hand,group_to_right_hand):
    ...
    should_to_hand_left = fast_mat_inv(should_to_sun_left_should)@fast_mat_inv(head_to_left_S)@fast_mat_inv(group_to_head)@group_to_left_hand@hand_to_sun_left_hand


    should_to_hand_right = fast_mat_inv(should_to_sun_right_should)@fast_mat_inv(head_to_right_S)@fast_mat_inv(group_to_head)@group_to_right_hand@hand_to_sun_right_hand
...
class TFPublisher:
    def __init__(self):
        ...
        while True:
            ...
            if l==16*4*53:
                ...
                vr_device_type='quest3_controller'
                xyzqwqxqyqz = quest3_controller.handle_raw_data(data_bytes)
                self.process_quest3_controller(xyzqwqxqyqz)
    ...
    def send_message(self,zyx_left_robot,
      ...
      # left_wrist_t[0]=np.clip(left_wrist_t[0],0,100)
        left_wrist_t[1]=np.clip(left_wrist_t[1],200,500)
        # left_wrist_t[2]=np.clip(left_wrist_t[2],0,100)

        # right_wrist_t[0]=np.clip(right_wrist_t[0],0,100)
        right_wrist_t[1]=np.clip(right_wrist_t[1],-500,-200)
        # right_wrist_t[2]=np.clip(right_wrist_t[2],0,100)
    ...
    def process_quest3_controller(self,xyzqwqxqyqz):
        ...
        [
            ...
            left_finger_1,left_finger_2,left_finger_3,left_finger_4,left_finger_5,left_finger_6,
            right_finger_1,right_finger_2,right_finger_3,right_finger_4,right_finger_5,right_finger_6
            ]=quest3_controller.process(xyzqwqxqyqz)
